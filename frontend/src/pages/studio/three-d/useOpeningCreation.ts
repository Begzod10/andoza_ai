...
import { computeOpeningRect, wallGeom } from "./openingGeometry";
...
export function useOpeningCreation(deps: {
  ...
  const { W, D, H, addElement, setPendingWindowSpot, setSelectedWall } = deps;
  ...
  return { wallGeom: (wallId: string) => wallGeom(wallId, W, D), computeOpeningRect: (
    g: { axis: 'X' | 'Z'; leftAlong: number; length: number },
    point: { x: number; y: number; z: number },
    widthMm: number, heightMm: number, isDoor: boolean,
  ) => computeOpeningRect(g, point, widthMm, heightMm, isDoor, H), createOpening };
}
