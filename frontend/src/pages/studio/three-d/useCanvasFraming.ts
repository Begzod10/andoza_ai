...
export function useCanvasFraming(params: {
  ...
  const { preset, cutaway, W, D, H, roomId, setPresetVersion } = params;
  ...
  useEffect(() => {
    ...
    camKeyRef.current = { roomId, cutaway };
    if (!prev) return;
    if (prev.roomId !== roomId || prev.cutaway !== cutaway) {
      setPresetVersion((n) => n + 1);
    }
  }, [roomId, cutaway, setPresetVersion]);

  return { canvasBoxRef, cam, initCam, interiorMaxDist, topMinDist, maxPolarAngle };
}
